...
class HotkeyManager:
    ...
    def _register_gnome_hotkey(self, key_combo: str, command: str) -> bool:
        ...
        try:
            ...
            custom_path = (
                "/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/likx/"
            )
    ...
    def unregister_all(self) -> None:
        ...
        if self.desktop_env == "gnome":
            try:
                schema = "org.gnome.settings-daemon.plugins.media-keys"
                custom_path = "/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/likx/"
                ...
                if result.returncode == 0:
                    ...
                    if custom_path in current:
                        # Remove our path
                        new_value = current.replace(f"'{custom_path}', ", "").replace(
                            f", '{custom_path}'", ""
                        )
                        subprocess.run(
                            [
