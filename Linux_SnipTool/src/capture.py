"""Screenshot capture module for Linux SnipTool with Wayland and X11 support."""
...
class CaptureResult:
    ...
    def __init__(self, success: bool, filepath: Optional[Path] = None, 
                 error: Optional[str] = None, pixbuf: Optional[object] = None):
...
def capture_fullscreen_wayland(delay: int = 0) -> CaptureResult:
    ...
    temp_file = f"/tmp/sniptool_{int(time.time())}.png"
    ...
    try:
        result = subprocess.run(['gnome-screenshot', '-f', temp_file], 
    ...
    # Try spectacle (KDE)
    try:
        result = subprocess.run(['spectacle', '-b', '-n', '-o', temp_file], 
                              capture_output=True, timeout=5)
        if result.returncode == 0 and os.path.exists(temp_file):
            pixbuf = GdkPixbuf.Pixbuf.new_from_file(temp_file)
            os.unlink(temp_file)
    ...
    return CaptureResult(False, error="No Wayland screenshot tool found. Install grim, gnome-screenshot, or spectacle.")
...
def capture_region_wayland(x: int, y: int, width: int, height: int, delay: int = 0) -> CaptureResult:
    ...
    try:
        ...
        result = subprocess.run(['grim', '-g', geometry, temp_file], 
                              capture_output=True, timeout=5)
        if result.returncode == 0 and os.path.exists(temp_file):
            pixbuf = GdkPixbuf.Pixbuf.new_from_file(temp_file)
            os.unlink(temp_file)
...
def capture_window_wayland(delay: int = 0) -> CaptureResult:
    ...
    try:
        result = subprocess.run(['gnome-screenshot', '-w', '-f', temp_file], 
                              capture_output=True, timeout=5)
        if result.returncode == 0 and os.path.exists(temp_file):
            pixbuf = GdkPixbuf.Pixbuf.new_from_file(temp_file)
            os.unlink(temp_file)
    ...
    try:
        result = subprocess.run(['spectacle', '-a', '-b', '-n', '-o', temp_file], 
                              capture_output=True, timeout=5)
        if result.returncode == 0 and os.path.exists(temp_file):
            pixbuf = GdkPixbuf.Pixbuf.new_from_file(temp_file)
            os.unlink(temp_file)
    ...
    return CaptureResult(False, error="Window capture not supported on Wayland without gnome-screenshot or spectacle")
    ...
    try:
        ...
        if window_id is None:
            ...
            if result.returncode != 0:
                return CaptureResult(False, error="Could not get active window. Is xdotool installed?")
        ...
        return CaptureResult(False, error="xdotool not installed. Install it with: sudo apt install xdotool")
...
def save_capture(result: CaptureResult, filepath: Optional[Path] = None,
                 format_str: Optional[str] = None) -> CaptureResult:
...
def capture(mode: CaptureMode, delay: int = 0, 
            region: Optional[Tuple[int, int, int, int]] = None,
            window_id: Optional[int] = None,
            auto_save: bool = False,
            copy_clipboard: bool = True) -> CaptureResult:
