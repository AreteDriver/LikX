#!/usr/bin/env python3
"""Linux SnipTool - Enhanced screenshot capture and annotation tool for Linux.

Usage:
    linux-sniptool [OPTIONS]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        prog="linux-sniptool",
        description="Linux SnipTool - A screenshot capture and annotation tool with Wayland support"
    )
    
    parser.add_argument(
        "--version",
        action="version",
        version=f"Linux SnipTool {__version__}"
    )
...
def main():
    """Main entry point for Linux SnipTool."""
    args = parse_args()
